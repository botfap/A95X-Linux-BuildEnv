...
#define CHUNK_TYPE_METADATA  0x4D24
...
static int get_chunk_header(mms_io_t *io, mmsh_t *this)
{
    ...
    this->chunk_type  &=0xff7f;//never care B flag in framing header,see 2.2.3.1.1
    ...
    case CHUNK_TYPE_RESET:
        ...
        break;
     case CHUNK_TYPE_METADATA:
        lprintf("chunk type:       CHUNK_TYPE_METADATA\n");
        lprintf("chunk length:     %d\n", this->chunk_length);
        break;       
    default:
...
static int get_header(mms_io_t *io, mmsh_t *this)
{
    ...
    while (1) {
        if ((ret = get_chunk_header(io, this)) == MMSH_SUCCESS) {
            if (this->chunk_type == CHUNK_TYPE_ASF_HEADER) {
            ...
            } else     if (this->chunk_type == CHUNK_TYPE_DATA) {
                ...
                if (this->chunk_length >0) {
                    if(this->chunk_length>sizeof(this->buf)){
                        lprintf("Other packet len = %d exceed the in_buffer size %zu\n",
                               this->chunk_length, sizeof(this->buf));
                        return MMSH_ERROR;
                    }
                    len = io_read(io, this->s, this->buf, this->chunk_length);
                    if(len!=this->chunk_length){
                        lprintf( "Read other chunk type data failed!\n");
                        return MMSH_ERROR;
                    }else{
                        lprintf("Skip chunk type %d \n", this->chunk_type);
                        //continue;
                    }
                }
            }
...
static int get_media_packet(mms_io_t *io, mmsh_t *this)
{
    ...
    if ((ret = get_chunk_header(io, this)) == MMSH_SUCCESS) {
        ...
        if (len == this->chunk_length) {
            ...
            return MMSH_SUCCESS;
        } else{     //do reconnect,by peter,2012,1121.  
            ...
            this->buf_size = 0;
            this->http_request_number = 1;
            lprintf("maybe server close connection,do reconnecting\n");
            if (!mmsh_connect_int(io, this, this->current_pos, 0)) {
                return MMSH_ERROR;
            }
            return MMSH_SUCCESS;
        }
