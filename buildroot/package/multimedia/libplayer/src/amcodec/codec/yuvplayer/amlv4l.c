...
#define V4LDEVICE_ionvideo_NAME  "/dev/video20"
...
int amlv4l_init(amvideo_dev_t *dev, int type,int width,int height,int fmt,int buffernum)
{
    ...
    if (dev->display_mode == 0) {
        ret = open(V4LDEVICE_ionvideo_NAME, O_RDWR | O_NONBLOCK);
        v4l->memory_mode = V4L2_MEMORY_DMABUF;
    }
    else if (dev->display_mode == 1) {
        ret = open(V4LDEVICE_amlvideo_NAME, O_RDWR | O_NONBLOCK);
...
int amlv4l_dequeue_buf(amvideo_dev_t *dev, vframebuf_t *vf)
{
    ...
    if (!ret && vbuf.index < v4l->buffer_num) {
        if (dev->display_mode == 1) {
            vf->pts = vbuf.timestamp.tv_usec;
            vf->pts = vf->pts * 100 /9;
            //LOGE("vf->pts=%lld, vbuf.timestamp.tv_usec=%d,vbuf.index=%d",vf->pts,vbuf.timestamp.tv_usec,vbuf.index);
        } else if (dev->display_mode == 0) {
...
int amlv4l_queue_buf(amvideo_dev_t *dev, vframebuf_t *vf)
{
    ...
    vbuf.m.fd = vf->fd;
    vbuf.length = vf->length;
    vbuf.timestamp.tv_sec = vf->pts >> 32;
    vbuf.timestamp.tv_usec = vf->pts & 0xFFFFFFFF;
