...
#define DEFAULT_STATE_ERROR AmlAsink_STATE_ERROR_NONE
...
#define GST_TYPE_AmlAsink_STATE_ERROR (gst_amlasink_state_error_get_type())
static GType
gst_amlasink_state_error_get_type (void)
{
  static GType amlasink_state_error_type = 0;
  static const GEnumValue fakesink_state_error[] = {
    {AmlAsinkSTATE_ERROR_NONE, "No state change errors", "none"},
    {AmlAsink_STATE_ERROR_NULL_READY,
        "Fail state change from NULL to READY", "null-to-ready"},
    {AmlAsink_STATE_ERROR_READY_PAUSED,
        "Fail state change from READY to PAUSED", "ready-to-paused"},
    {AmlAsink_STATE_ERROR_PAUSED_PLAYING,
        "Fail state change from PAUSED to PLAYING", "paused-to-playing"},
    {AmlAsink_STATE_ERROR_PLAYING_PAUSED,
        "Fail state change from PLAYING to PAUSED", "playing-to-paused"},
    {AmlAsink_STATE_ERROR_PAUSED_READY,
  ...
  if (!amlasink_state_error_type) {
    amlasink_state_error_type =
        g_enum_register_static ("GstAmlAsinkStateError", amlasink_state_error);
  }
  return amlasink_state_error_type;
}

#define _do_init \
    GST_DEBUG_CATEGORY_INIT (gst_amlasink_debug, "amlasink", 0, "amlasink element");
#define gst_amlasink_parent_class parent_class
G_DEFINE_TYPE_WITH_CODE (GstAmlAsink, gst_amlasink, GST_TYPE_BASE_SINK,
    _do_init);

static void gst_amlasink_set_property (GObject * object, guint prop_id,
    const GValue * value, GParamSpec * pspec);
static void gst_amlasink_get_property (GObject * object, guint prop_id,
    GValue * value, GParamSpec * pspec);
static void gst_amlasink_finalize (GObject * obj);

static GstStateChangeReturn gst_amlasink_change_state (GstElement * element,
    GstStateChange transition);

static GstFlowReturn gst_amlasink_preroll (GstBaseSink * bsink,
    GstBuffer * buffer);
static GstFlowReturn gst_amlasink_render (GstBaseSink * bsink,
    GstBuffer * buffer);
static gboolean gst_amlasink_event (GstBaseSink * bsink, GstEvent * event);
static gboolean gst_amlasink_query (GstBaseSink * bsink, GstQuery * query);

static guint gst_amlasink_signals[LAST_SIGNAL] = { 0 };
...
gst_amlasink_class_init (GstAmlAsinkClass * klass)
{
   ...
   */
  gst_amlasink_signals[SIGNAL_HANDOFF] =
      g_signal_new ("handoff", G_TYPE_FROM_CLASS (klass), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET (GstAmlAsinkClass, handoff), NULL, NULL,
   ...
   * This signal gets emitted before unreffing the buffer.
   */
  gst_amlasink_signals[SIGNAL_PREROLL_HANDOFF] =
      g_signal_new ("preroll-handoff", G_TYPE_FROM_CLASS (klass),
      G_SIGNAL_RUN_LAST, G_STRUCT_OFFSET (GstAmlAsinkClass, preroll_handoff),
  ...
  gstbase_sink_class->event = GST_DEBUG_FUNCPTR (gst_amlasink_event);
  gstbase_sink_class->preroll = GST_DEBUG_FUNCPTR (gst_amlasink_preroll);
  gstbase_sink_class->render = GST_DEBUG_FUNCPTR (gst_amlasink_render);
  gstbase_sink_class->query = GST_DEBUG_FUNCPTR (gst_amlasink_query);
}

static void
gst_amlasink_init (GstAmlAsink * fakesink)
{
  amlasink->silent = DEFAULT_SILENT;
  amlasink->dump = DEFAULT_DUMP;
  amlasink->last_message = g_strdup (DEFAULT_LAST_MESSAGE);
  amlasink->state_error = DEFAULT_STATE_ERROR;
  amlasink->signal_handoffs = DEFAULT_SIGNAL_HANDOFFS;
  amlasink->num_buffers = DEFAULT_NUM_BUFFERS;

  gst_base_sink_set_sync (GST_BASE_SINK (amlasink), DEFAULT_SYNC);
}

static void
gst_amlasink_finalize (GObject * obj)
{
    ...
    const GValue * value, GParamSpec * pspec)
{
  ...
  sink = GST_AmlAsink(object);

  switch (prop_id) {
    case PROP_STATE_ERROR:
      sink->state_error = (GstAmlAsinkStateError) g_value_get_enum (value);
      break;
    case PROP_SILENT:
...
static void
gst_amlasink_get_property (GObject * object, guint prop_id, GValue * value,
    GParamSpec * pspec)
{
  GstAmlAsink *sink;
...
static void
gst_amlasink_notify_last_message (GstAmlAsink * sink)
{
  g_object_notify_by_pspec ((GObject *) sink, pspec_last_message);
}

static gboolean
gst_amlasink_event (GstBaseSink * bsink, GstEvent * event)
{
  GstAmlAsink *sink = GST_AmlAsink (bsink);
...
gst_amlasink_preroll (GstBaseSink * bsink, GstBuffer * buffer)
{
  GstAmlAsink *sink = GST_AmlAsink (bsink);
  ...
  if (!sink->silent) {
    ...
    gst_amlasink_notify_last_message (sink);
  }
...
static GstFlowReturn
gst_amlasink_render (GstBaseSink * bsink, GstBuffer * buf)
{
  GstAmlAsink*sink = GST_AmlAsink_CAST (bsink);
  ...
  if (sink->signal_handoffs)
    g_signal_emit (sink, gst_amlasink_signals[SIGNAL_HANDOFF], 0, buf,
        bsink->sinkpad);
...
static gboolean
gst_amlasink_query (GstBaseSink * bsink, GstQuery * query)
{
  gboolean ret;
...
gst_amlasink_change_state (GstElement * element, GstStateChange transition)
{
  ...
  switch (transition) {
    case GST_STATE_CHANGE_NULL_TO_READY:
      if (amlasink->state_error == AmlAsink_STATE_ERROR_NULL_READY)
        goto error;
      break;
    case GST_STATE_CHANGE_READY_TO_PAUSED:
      if (amlasink->state_error == AmlAsink_STATE_ERROR_READY_PAUSED)
        goto error;
      amlasink->num_buffers_left = amlasink->num_buffers;
      break;
    case GST_STATE_CHANGE_PAUSED_TO_PLAYING:
      if (amlasink->state_error == AmlAsink_STATE_ERROR_PAUSED_PLAYING)
        goto error;
      break;
    default:
      break;
  ...
  switch (transition) {
    case GST_STATE_CHANGE_PLAYING_TO_PAUSED:
      if (amlasink->state_error == AmlAsink_STATE_ERROR_PLAYING_PAUSED)
        goto error;
      break;
    case GST_STATE_CHANGE_PAUSED_TO_READY:
      if (amlasink->state_error == AmlAsink_STATE_ERROR_PAUSED_READY)
        goto error;
      break;
    case GST_STATE_CHANGE_READY_TO_NULL:
      if (amlasink->state_error == AmlAsink_STATE_ERROR_READY_NULL)
        goto error;
      GST_OBJECT_LOCK (fakesink);
      g_free (amlasink->last_message);
      amlasink->last_message = NULL;
...
{
  return gst_element_register (plugin, "amlasink", GST_RANK_SECONDARY, gst_amlasink_get_type ());
}
